...
private:
  ...
  Ptr<MmWaveIndicationMessageHelper>  BuildRicIndicationMessageCuUp(std::string plmId, std::string gnbId);
  Ptr<MmWaveIndicationMessageHelper>  BuildRicIndicationMessageCuCp(std::string plmId, std::string gnbId);
  Ptr<MmWaveIndicationMessageHelper> BuildRicIndicationMessageDu(std::string plmId, uint16_t nrCellId, std::string gnbId);
  ...
  std::map<uint64_t, std::vector<uint16_t>> ue_nbcell_id;
  std::map<uint64_t, std::vector<double>> ue_nbcell_sinr;
  std::map<uint64_t, double> ue_cell_sinr;
  std::map<uint64_t, long> ue_prb_usage;
  std::map<uint64_t, double> ue_pdcp_dl;
  std::map<uint64_t, double> ue_pdcp_ul;
  long cell_ava_prb_dl;
  long cell_ava_prb_ul;
  double cell_pdcp_dl;
  double cell_pdcp_ul;
